...
class Block {
  ...
  calculateHash(nonce) {
    ...
    const transactionString = (
      this.transactions.map(({ signature }) => signature).join('')
    );

    const message = this.previousHash + transactionString + nonce;
...
class Blockchain {
  ...
  getBalance(publicKey) {
    ...
    this.blocks.forEach(({ transactions }) => {
